...
assert.equal(pickUnclearIntentReply("clarify", "ru"), WA_UNCLEAR_CLARIFY_RU);
assert.equal(isUnclearClarifyAsk(WA_UNCLEAR_CLARIFY_RU), true);
...
assert.equal(
  inboundLooksLikeClearKnowledgeQuestion(
    "Можно перенести несколько занятий с следующего месяца на этот?"
  ),
  true
);
