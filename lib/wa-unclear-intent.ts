...
import { UNKNOWN_KNOWLEDGE_TEAM_HANDOFF_REPLY, UNKNOWN_KNOWLEDGE_TEAM_HANDOFF_REPLY_EN } from "@/lib/wa-unknown-knowledge-handoff";
...
export function buildUnclearIntentPromptRule(alreadyAsked: boolean): string {
  ...
  return `- אם לא הבנת עד הסוף למה הליד מתכוון (הודעה מעורפלת, חסרה, או לא ברורה) — עני רק: «${WA_UNCLEAR_CLARIFY_HE}» (או באנגלית: «${WA_UNCLEAR_CLARIFY_EN}»). פעם אחת בלבד בשיחה. זה לא חוסר ידע: אם ברור מה שואלים ואין מידע בידע — עני רק «${UNKNOWN_KNOWLEDGE_TEAM_HANDOFF_REPLY}» (EN: «${UNKNOWN_KNOWLEDGE_TEAM_HANDOFF_REPLY_EN}»), לא המשפט הזה.`;
}
