...
export function buildUnclearIntentPromptRule(alreadyAsked: boolean): string {
  ...
  return `- אם לא הבנת עד הסוף למה הליד מתכוון (הודעה מעורפלת, חסרה, או לא ברורה) — עני רק: «${WA_UNCLEAR_CLARIFY_HE}» (EN: «${WA_UNCLEAR_CLARIFY_EN}»; RU: «${WA_UNCLEAR_CLARIFY_RU}»). פעם אחת בלבד בשיחה. זה לא חוסר ידע: אם ברור מה שואלים ואין מידע בידע — «אין לי את הפרטים על כך - אני מעבירה את הבקשה לצוות», לא המשפט הזה. הודעת שיווק/הפניה שהליד לא שאל עליה — אותו כלל, אל תמשיכי אותה כשיחה. ברכת חולין («היי מה קורה», «מה נשמע», «מה המצב», «מה הולך», «מה העניינים») זה לא חוסר הבנה — עני בחביבות: «היי! מעולה, איך אפשר לעזור?». אל תבקשי לנסח מחדש.
- אסור לכתוב THOUGHT / Reasoning / מחשבות בגוף התשובה. רק ההודעה ללקוח.`;
}
