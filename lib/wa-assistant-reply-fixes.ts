...
- איות: «אימון» לא «אימן»; «להצטרף» לא «להצטפן»; «מבטלים» לא «ובוטלים»; «לומדים» לא «למדים»; «אין לי» לא «לא יש לי»; «בהצלחה» לא «מצליחה»; «החלמה מהירה» לא «בהחלמה מהירה»; «בדיוק» לא «לבדיוק»; «לוודא» לא «ליוודע»; «יכולה» לא «יוכלה»; «מתאים לך» לא «מתוקף לך»; «רצפת האגן» לא «הרצפה האגן»; «להגיד לה» לא «להגידה לה».
...
- איחור / בדרך לשיעור: «אין בעיה בכלל! 🙂 אנחנו כאן» ואם ציינו כמה דקות — הוסיפי רק «נראה אותך בעוד X דקות». אסור «קח את הזמן», «בטוח שזה יעבוד», «עד עכשיו», אסור עוד משפט.
...
${
  waCtx.scheduleInterestServiceName?.trim()
    ? "- אם השירות שנבחר כרגע לא מתאים לליד (גיל / קהל / רמה) ויש שירות אחר ברשימת האימונים שכן מתאים: צייני במפורש את שם השירות האחר מהרשימה (אות-באות). אל תשאלי מועדים של השירות הישן. המערכת תעביר לשירות המתאים."
    : ""
}`;
}
...
const CHEERLEADING_RE =
  /בטוח\s+שזה\s+יעבוד|קח(?:י|ו)?\s+את\s+הזמן(?:\s+שצריך|\s+שלך)?|עד\s+עכשיו!?/iu;
