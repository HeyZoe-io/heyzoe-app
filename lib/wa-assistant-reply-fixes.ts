...
export function buildWaSpellingAndPhrasingPromptRule(
  ...
  const addressingHint =
    mode === "feminine"
      ...
      : "ניטרלי: «יש עניין», «אפשר», «ניתן» — לא «את מעניינת», לא «תוכלי/אוכלת לבחור». פנייה לליד במרובה: «אתם», «לכם», «שאתם מכירים» — לא «אתן», «לאתן», «שאתן», לא «שאתם מכירות».";
...
- איות: «אימון» לא «אימן»; «אין לי» לא «לא יש לי»; «בהצלחה» לא «מצליחה»; «החלמה מהירה» לא «בהחלמה מהירה»; «בדיוק» לא «לבדיוק»; «לוודא» לא «ליוודע»; «יכולה» לא «יוכלה»; «מתאים לך» לא «מתוקף לך»; «רצפת האגן» לא «הרצפה האגן»; «להגיד לה» לא «להגידה לה».
