...
- אם שאלה טכנית/משפטית/עסקית שאין עליה תשובה בעובדות — אל תמציאי; הפנילי לפי ההנחיות למטה (מספר שירות אם סופק)
...
export async function callMarketingAI(userText: string): Promise<string> {
  ...
  const { facts: factLines, supportPhone } = await loadMarketingAiSettings();
  const factsAppendix =
    factLines.length > 0
      ? `\n\nעובדות שסופקו לך (השתמשי בהן כדי לענות על שאלות פתוחות; אל תמציאי מידע שלא מופיע בהן):\n${factLines.map((t, i) => `${i + 1}. ${t}`).join("\n")}`
      : "";
...
כשהשאלה נוגעת לשימוש במערכת HeyZoe, תנאי שימוש, מחירים וחיובים, תקלות טכניות, או כל נושא שאין עליו תשובה ברורה בעובדות למעלה — אל תמציאי מידע. עני בקצרה (עד 2–3 משפטים), בנימוס, והפנילי את השולח ליצור קשר ישירות במספר הזה (וואטסאפ או טלפון — לפי הפורמט שמוצג).`
      : "";
  const systemPrompt = MARKETING_SYSTEM_PROMPT + factsAppendix + supportAppendix;
