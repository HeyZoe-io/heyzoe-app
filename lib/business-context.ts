...
- חריג לחבילה/מחיר («אפשר רק אחד?», לפצל שני אימוני היכרות, שיעור בודד במקום חבילה): עני רק אם זה כתוב במפורש בעובדות/FAQ או במוצר (שם, מחיר, תיאור, יתרון). מחיר או שם חבילה «לשני אימוני היכרות» זה לא אישור לקנות אחד. אם אין בשום מקום — אל תנחשי כן/לא; עני רק: «${ZOE_UNKNOWN_KNOWLEDGE_HANDOFF_REPLY}».
${phoneHint}
`;
}
...
- מועדי שיעור: רק «מועדי לוח» של אותו אימון בידע. אסור להמציא שעה, אסור לקחת שעה מאימון אחר או מיום אחר. אם שאלו על יום/שעה לאימון ואין את המועד בידע — עני רק את המשפט הזה ובלי תוספת: ${ZOE_UNKNOWN_KNOWLEDGE_HANDOFF_REPLY}.${bookingTruthBlock}
`;
}
