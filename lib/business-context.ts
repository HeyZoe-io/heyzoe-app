...
import { UNKNOWN_KNOWLEDGE_TEAM_HANDOFF_REPLY } from "@/lib/wa-unknown-knowledge-handoff";
import {
...
${staticBlock || `רק אם באמת אין מידע: עני רק «${UNKNOWN_KNOWLEDGE_TEAM_HANDOFF_REPLY}». אסור «לא יש לי», «לא יש לי את הפרטים» או «לא יש מידע». אל תכתבי את המילה «התנצלות» כחלק מהתשובה; אל תמציאי ואל תנחשי.`}
- בכל מקרה של חוסר ידע, אסור לנסח «התנצלות ש...» או «התנצלות שלא...»; זה לא משפט תקין בעברית.
- אסור «לא יש לי» / «לא יש לי את הפרטים». חוסר ידע = רק «${UNKNOWN_KNOWLEDGE_TEAM_HANDOFF_REPLY}».
- חריג לחבילה/מחיר («אפשר רק אחד?», לפצל שני אימוני היכרות, שיעור בודד במקום חבילה): עני רק אם זה כתוב במפורש בעובדות/FAQ או במוצר (שם, מחיר, תיאור, יתרון). מחיר או שם חבילה «לשני אימוני היכרות» זה לא אישור לקנות אחד. אם אין בשום מקום — אל תנחשי כן/לא; עני רק: «${UNKNOWN_KNOWLEDGE_TEAM_HANDOFF_REPLY}».
${phoneHint}
`;
}
...
- מועדי שיעור: רק «מועדי לוח» של אותו אימון בידע. אסור להמציא שעה, אסור לקחת שעה מאימון אחר או מיום אחר. אם שאלו על יום/שעה לאימון ואין את המועד בידע — עני רק את המשפט הזה ובלי תוספת: ${UNKNOWN_KNOWLEDGE_TEAM_HANDOFF_REPLY}.${bookingTruthBlock}
`;
}
