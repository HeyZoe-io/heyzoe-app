/** «תתקשרו» / «תתקשרי אלי» — הליד מבקשת שהסטודיו יתקשר אליה (לא שהיא תתקשר). */
export function userAskedStudioToCallThem(userText: string): boolean {
  const t = String(userText ?? "")
    .trim()
    .replace(/[.!?]+$/g, "")
    .replace(/\s+/g, " ");
  if (!t) return false;
  if (/אל\s+(?:תתקשר|התקשר)/u.test(t)) return false;
  return /^(?:בבקשה\s+)?(?:תתקשר(?:ו|י)|התקשר(?:ו|י))(?:\s+אלי(?:י)?)?(?:\s+בבקשה)?$/u.test(t);
}
...
export function userRequestedHumanAgent(userText: string): boolean {
  ...
  if (!raw) return false;
  if (userAskedStudioToCallThem(raw)) return true;
  const t = raw.toLowerCase();
  const hebrew =
    /נציג|נציגה|בן\s*אדם|אדם\s*אמיתי|מענה\s*אנושי|דברו\s*איתי|לדבר\s*עם\s*מישהו|לדבר\s*עם\s*אדם|העבר(ה|י)\s*ל|תחבר(ו|י)\s*אותי|אפשר\s*לדבר\s*עם|מישהו\s*אמיתי|נציג\s*אנושי|שירות\s*אנושי|לא\s*רובוט|לא\s*בוט|עם\s*בשר\s*ודם|(אני\s*)?(רוצה|צריך|צריכה|מעוניין|מעוניינת|מבקש|מבקשת).{0,50}שירות\s*לקוחות|שירות\s*לקוחות.{0,20}(בבקשה|עכשיו)|(?:יצירת|ליצור|ש(?:יצרו|יצור|תיצור)|יצרו)\s*(?:אית(?:י|נו|ך)|לי)?\s*קשר|אשמח\s+ש(?:יצרו|יצור)\s*(?:אית(?:י|נו)|לי)\s*קשר|(?:תחזר|חזר)[ור]?ה?\s+אלי|(?:תתקשר|התקשר)[ו]?י\s+אלי|ליצירת\s+קשר|אפשר\s+שיחז(?:רו|ור)(?:\s+אלי(?:י)?)?/i.test(
      raw
    );
  const english =
