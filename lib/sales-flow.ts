...
נשמח לפגוש אותך ביום ____ בשעה ______ (מלאי מתוך פרטי האימון בארבוקס כשהם ידועים בשיחה או בנתוני ההזמנה).

בכתובת: ____ (מלאי משדה «כתובת» בפרטי העסק בידע).
...
נשמח לפגוש אתכם ביום ____ בשעה ______ (מלאי מפרטי האימון בארבוקס כשהם ידועים).

הכתובת: ____ (משדה הכתובת בפרטי העסק בידע).
...
export function formatSalesFlowForPrompt(
  ...
  const benefitLines = named
    .map((n) => {
      ...
      return `  - ${n}: תיאור קצר מההגדרות (משפט אחד חי וברור — לא רשימה): ${b}`;
    })
    .join("\n");
