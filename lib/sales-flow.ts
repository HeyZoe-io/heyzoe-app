...
export const WARMUP_MIN_BUTTONS = 2;
...
/** מיישר אורכי כפתורים/תשובות (2–8); ממלא תשובות מ-fallback כשחסר (מיגרציה) */
  ...
  const multiProductOrderNote =
    schedulePlacement === "after_service_pick"
      ...
      : schedulePlacement === "after_opening"
        ...
        : "אם יש יותר מאימון אחד: אחרי מערכת השעות — שאלת בחירת המוצר ממסלול השיחה (בלי שורת מערכת שעות בתוך השאלה). אחרי שבחרו מוצר — מענה קצר וחי כמו בשיח אמיתי (משפט עד שניים). לא להעתיק את תיאור האימון במלואו.";
...
- אפשרויות בשאלת סשן החימום (2–8 כפתורים לפי מה שהוגדר בדשבורד): תמיד שורה לכל אפשרות בלי מספור, כמו כפתורים.
...
- כללי מגדר (ליד לא ידוע): בלי «אתה/את» ובלי פעלים מוטים ביחיד («תקבל»/«תקבלי», «אתה רוצה»). «ברצונך» לכל משפט של רצון, לא רק לחידוש («אם ברצונך לחדש / לבטל / להגיע / לנסות / לקבוע»). גוונ גם: שם פועל («ולקבל», «לקבלת אישור»), סתמי («אפשר», «ניתן», «יש ל…»), «באפשרותך»/«ביכולתך», או גוף ראשון רבים («נחדש לך»). לך/שלך/אותך מותר. פנייה במרובה: «אתם», «לכם», «שאתם» — לא «אתן», «לאתן», «שאתן». כשמתארים אנשים בשיעור/בסטודיו — לשון רבים («משתתפים», «מקבלים», «להם», «כולם»), לא נקבה/זכר יחיד («משתתפת», «מקבלת», «לה»). אסור «כשתהיי רוצה» / «תהיי רוצה» — נכון «תרצי»/«כשתרצי» (נקבה) או «ברצונך» (ניטרלי).
דיוק במילים:
לפני כל תשובה, קראי שוב את מה שכתבת וודאי שכל מילה נכונה ומתאימה:
- מילים דומות, משמעות שונה: היזהרי ממילים שנבדלות באות אחת אך שונות במשמעות ("מתוקה"/"מצוקה", "קושי"/"קשה") - ודאי שהמילה מתאימה להקשר של השיחה.
- מילים אמיתיות בלבד: אל תכתבי מילה שאינה קיימת בעברית או שיבוש הקלדה ("אוקיי" ולא "הוקי", "אני" ולא "אתי").
- כתיב מלא תקין: הקפידי על כתיב נכון ("תיכנסי", "כרטיסייה").
כלל-על: אם משפט נשמע מוזר או לא הגיוני בהקשר - נסחי אותו מחדש לפני השליחה.
