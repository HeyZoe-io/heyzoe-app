...
export type SalesFlowConfig = {
  ...
  /** בחירת פלואו לסשן פתיחה/חימום בטאב «שיחה» — "quiz" מזריק תוכן קבוע (שם עסק/מחיר/כתובת מוקפאים בזמן היצירה) */
  warmup_style?: "short" | "quiz";
  /**
   * מיקום סשן מערכת שעות במסלול השיחה.
...
export function resolveScheduleBoardPlacement(
  cfg: Pick<SalesFlowConfig, "schedule_board_placement"> | null | undefined
): ScheduleBoardPlacement {
  const p = cfg?.schedule_board_placement;
  if (p === "after_opening" || p === "after_service_pick" || p === "before_service_pick") return p;
  return "before_service_pick";
}
...
export function parseSalesFlowFromSocial(raw: unknown): SalesFlowConfig | null {
  ...
  const cfg: SalesFlowConfig = {
    ...
    schedule_board_placement: resolveScheduleBoardPlacement({
      schedule_board_placement:
        o.schedule_board_placement === "after_opening" ||
        o.schedule_board_placement === "after_service_pick" ||
        o.schedule_board_placement === "before_service_pick"
...
export type FormatSalesFlowPromptOpts = {
  /** false = סשן חימום הוסר בדשבורד — אל תזריקי שאלה/כפתורים לפרומפט */
  warmupSessionEnabled?: boolean;
};

function salesFlowScheduleBoardOrderNote(
  placement: ScheduleBoardPlacement,
  warmupOn: boolean
): string {
  if (placement === "after_opening") {
    return warmupOn
      ? "מערכת השעות נשלחת **אוטומטית** אחרי סיום סשן הפתיחה (לפני סשן החימום)."
      : "מערכת השעות נשלחת **אוטומטית** אחרי סיום סשן הפתיחה (לפני בחירת מוצר).";
  }
  if (placement === "after_service_pick") {
    return "מערכת השעות נשלחת **אוטומטית** אחרי בחירת מוצר (לא לפני תפריט הבחירה).";
  }
  return warmupOn
    ? "אחרי סיום החימום (כולל שאלות נוספות אם הוגדרו) — המערכת שולחת **אוטומטית** את מערכת השעות (תמונה/קישור), לפני בחירת מוצר."
    : "אחרי סיום הפתיחה — המערכת שולחת **אוטומטית** את מערכת השעות (תמונה/קישור), לפני בחירת מוצר.";
}

function salesFlowMultiProductOrderNote(
  placement: ScheduleBoardPlacement,
  warmupOn: boolean
): string {
  if (placement === "after_service_pick") {
    return warmupOn
      ? "אם יש יותר מאימון אחד: אחרי החימום — שאלת בחירת המוצר. אחרי שבחרו מוצר — מענה קצר, ואז מערכת השעות נשלחת אוטומטית."
      : "אם יש יותר מאימון אחד: אחרי הפתיחה — שאלת בחירת המוצר. אחרי שבחרו מוצר — מענה קצר, ואז מערכת השעות נשלחת אוטומטית.";
  }
  if (placement === "after_opening") {
    return warmupOn
      ? "אם יש יותר מאימון אחד: מערכת השעות כבר נשלחה אחרי הפתיחה; אחרי החימום — שאלת בחירת המוצר. אחרי שבחרו מוצר — מענה קצר וחי כמו בשיח אמיתי."
      : "אם יש יותר מאימון אחד: מערכת השעות כבר נשלחה אחרי הפתיחה; אחר כך — שאלת בחירת המוצר. אחרי שבחרו מוצר — מענה קצר וחי כמו בשיח אמיתי.";
  }
  return "אם יש יותר מאימון אחד: אחרי מערכת השעות — שאלת בחירת המוצר ממסלול השיחה (בלי שורת מערכת שעות בתוך השאלה). אחרי שבחרו מוצר — מענה קצר וחי כמו בשיח אמיתי (משפט עד שניים). לא להעתיק את תיאור האימון במלואו.";
}
...
export function formatSalesFlowForPrompt(
  ...
  address = "",
  directions = "",
  opts?: FormatSalesFlowPromptOpts
): string {
      ...
      const hint =
        b.kind === "schedule"
          ? (b.schedule_cta_delivery ?? "link") === "image" && String(b.schedule_cta_image_url ?? "").trim()
            ? "כשמשתמש בוחר: תמונת מערכת שעות (מ־CTA או מטאב לינקים); בלי לינק אם יש תמונה"
  ...
  const warmupOn = opts?.warmupSessionEnabled !== false;
  const schedulePlacement = resolveScheduleBoardPlacement(c);
  const scheduleBoardOrderNote = salesFlowScheduleBoardOrderNote(schedulePlacement, warmupOn);
  const multiProductOrderNote = salesFlowMultiProductOrderNote(schedulePlacement, warmupOn);
  const warmupScheduleContextNote =
    schedulePlacement === "after_opening"
      ? "תוכן סשן החימום (אחיד לכל המוצרים — נשלח אחרי מערכת השעות ולפני בחירת מוצר):"
      : schedulePlacement === "after_service_pick"
        ? "תוכן סשן החימום (אחיד לכל המוצרים — נשלח לפני בחירת מוצר; מערכת השעות אחרי הבחירה):"
        : "תוכן סשן החימום (אחיד לכל המוצרים — נשלח לפני מערכת השעות ובחירת מוצר):";
  const afterServicePickToneRules = `- התאימי את הרוח לסוג האימון שנבחר: אקרו/דינמי - אנרגיה, קהילה, אתגר חיובי; פילאטיס/מכשירים - חיטוב, התחזקות, השקעה בעצמך; יוגה/מיינדפולנס - חיבור פנימי, איזון, גוף־נפש. אפשר לפתוח ב"אוקיי מדהים!" או "איזה כיף :)" לפי הטון.
...
- כללי מגדר (ליד לא ידוע): בלי «אתה/את» בפנייה ישירה. פנייה במרובה: «אתם», «לכם», «שאתם» — לא «אתן», «לאתן», «שאתן». כשמתארים אנשים בשיעור/בסטודיו — לשון רבים («משתתפים», «מקבלים», «להם», «כולם»), לא נקבה/זכר יחיד («משתתפת», «מקבלת», «לה»).
...
- מציין מסלול (הנחיה בלבד, לא טקסט ללקוח): ${c.after_service_pick}`;
  const warmupSessionPromptBlock = warmupOn
...
- ${multiProductOrderNote}
${afterServicePickToneRules}

${warmupScheduleContextNote}
...
${formatExtraSteps("שאלות נוספות בסשן חימום (לפני ההנעה לפעולה)", c.opening_extra_steps)}`
    : `סשן חימום אינו פעיל בעסק הזה — דלגי עליו לגמרי. אסור לשאול שאלת מטרות/ציפיות מהאימון, אסור לפרט אפשרויות כפתורי חימום בטקסט, ואסור לפתוח ב«לפני שנעבור להרשמה» ואז שאלת המשך כזאת.
- ${scheduleBoardOrderNote}
- ${multiProductOrderNote}
- אם יש כמה מוצרים והלקוח עדיין לא בחר — אל תסיקי בחירת מוצר משאלה פתוחה (גם אם הזכיר ספורט, בן/בת זוג או רצון לנסות). המערכת שולחת את תפריט בחירת המוצר בנפרד. רק אחרי בחירה אמיתית בתפריט:
${afterServicePickToneRules}`;
  const warmupButtonRules = warmupOn
...
`
    : "";
  const greetingExtrasTitle = warmupOn
    ? "שאלות נוספות מיד אחרי טקסט הפתיחה (לפני סשן החימום)"
    : "שאלות נוספות מיד אחרי טקסט הפתיחה (לפני בחירת מוצר)";
...
- בהודעות דטרמיניסטיות של הפלואו (כפתורים): מענה קצר לשלב הנוכחי + השאלה הבאה + אפשרויות בחירה.
...
${warmupButtonRules}- אם יש רק שירות אחד בפלואו - דלגי על שאלת בחירה בין מוצרים (אחרי מערכת השעות).
- אם הלקוח כותב בצ׳אט חופשי באמצע הפלואו: עני בקצרה מהידע (Claude) בלי השאלה הבאה ובלי אפשרויות בטקסט. אם זו כוונה/עדכון בלי שאלה — אישור קצר בלבד, בלי שיעור חיים ובלי «בואי תרשמי». המערכת מחזירה את השאלה/CTA בנפרד (אל תוסיפי אותם בגוף התשובה).
- אם מפרטים מועדי שיעור (יום + שעה) בגוף התשובה — סיימי ב«מתי נוח לך להגיע?». אסור לצרף אחרי זה הודעת «נרשמת בהצלחה» / כתובת / הגעה / אינסטגרם.
- אחרי בחירת יום/שעה: אישור קצר בלבד. המערכת שולחת לינק הרשמה בנפרד. אסור לשלוח «נרשמת בהצלחה» יחד עם לינק תשלום.
...
${formatExtraSteps(greetingExtrasTitle, c.greeting_extra_steps)}

${warmupSessionPromptBlock}
...
- התבנית למטה נשלחת **על ידי המערכת בלבד** אחרי שהלקוח באמת השלים תשלום/הרשמה («נרשמתי» או webhook). אל תעתיקי אותה כששולחים לינק הרשמה, אחרי בחירת מועד, או כאישור שנרשמו.
...
תבנית והנחיות ממסלול המכירה (לא לשלוח לפני הרשמה אמיתית):
