...
function matchesGroupCRegistration(t: string): boolean {
  ...
  if (
    new RegExp(
      `${p}(?:ב)?רוצה\\s+(?:להירשם|להצטרף|לנסות|אימון\\s+(?:ניסיון|נסיון|היכרות|הכרות)|שיעור\\s+(?:ניסיון|נסיון|היכרות|הכרות)|לקבוע(?:\\s+אימון)?)$`,
      "u"
    ).test(t)
  ) {
    return true;
  ...
  if (
    new RegExp(
      `${p}אפשר\\s+(?:להירשם|אימון\\s+(?:ניסיון|נסיון|היכרות|הכרות)|שיעור\\s+(?:ניסיון|נסיון|היכרות|הכרות))$`,
      "u"
    ).test(t)
  ) {
    return true;
