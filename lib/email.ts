...
export function dailySummaryOwnerEmail(input: {
  ...
  human_requested_leads?: DailySummaryIdleLead[];
  ...
  const humanRequestedLeads = input.human_requested_leads ?? [];
  ...
  const statsBlock = [
    ...
    `ביקש נציג: ${formatDailySummaryLeadListLine(humanRequestedLeads)}`,
