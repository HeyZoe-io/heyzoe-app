...
- "מסלול מכירה" - הגדרת הבוט, מחולקת ל-5 שלבים (ראו למטה).
- "שיחות" - צפייה בשיחות של זואי מול לידים (היסטוריית הודעות לפי מספר).
- "לידים" - ניהול הלידים שנכנסו: סינון לפי תאריך וסטטוס, שינוי סטטוס ידני (נרשם / לא רלוונטי / ללא מענה / ביקש נציג). אם ליד כבר נרשם ואז ביקש נציג — הסטטוס הופך ל«ביקש נציג + נרשם» ונשלחת התראה לצוות.
- "אנליטיקס" - נתונים וביצועים: לידים חדשים (רק מי שפתח פלואו מכירה מול זואי, לא כל שיחה נכנסת), המרות, שיעור המרה. בחבילת Pro גם: לידים לפי יום, שעות שיא, חזרו אחרי פולואפ, אימונים פופולריים, פערי ידע.
- "אוטומציות" (כתובת /templates) - ניהול תבניות וואטסאפ וטריגרים אוטומטיים (ראו למטה).

תפריט חשבון (אייקון פרופיל בפינה השמאלית):
- "פרטים אישיים" - שם, אימייל, טלפון.
- "חיוב וחבילות" - צפייה בחבילה הנוכחית, שדרוג/שינוי חבילה, ביטול מנוי (פעולה רגישה — להעביר לתמיכה).
- "התראות" - הגדרת התראות לבעל העסק: ליד נרשם, ליד ביקש נציג אנושי, סיכום יומי (בוואטסאפ ו/או במייל). כדי לקבל התראות בוואטסאפ צריך לחבר את המספר של הבעלים.
- "משתמשים" - הוספת משתמשים נוספים לדשבורד.
- "צור קשר" - פנייה לצוות HeyZoe.

מסלול מכירה — 5 שלבים:
  1. "לינקים" - לינק לאתר (סריקה אוטומטית), קישורי מערכת (מערכת שעות, מנויים, הרשמה ישירה), חיבור CRM (ארבוקס וכו'), אינסטגרם.
  2. "על העסק" - מספרי טלפון, מספר הוואטסאפ המחובר, הפעלה/כיבוי של זואי, זהות העסק (שם עסק, שם הבוט, תיאור), מיקום (כתובת והגעה), ידע כללי לזואי.
  ...
  4. "שיחה" - הניסוחים שזואי אומרת ללידים:
     - סשן פתיחה (טקסט ומדיה).
     - סשן חימום (שאלות לפני בחירת מוצר).
     - סשן מערכת שעות — ניתן לגרור למיקום: אחרי הפתיחה, לפני בחירת המוצר, או אחרי בחירת המוצר.
     - בחירת מוצר (כשיש יותר מאחד).
     - בחירת מועד (יום ושעה בשיחה — גם כשמערכת השעות לא אינטראקטיבית).
     - הנעה לפעולה (CTA) — שלושה כפתורים: כפתור 1 יכול להיות "הרשמה לשיעור ניסיון" או "שיחת מכירה" (הליד בוחר יום ובלוק שעות, ונשלחת התראה לבעלים). כפתורים 2–3: מערכת שעות ומחירי מנויים.
     - אחרי הרשמה — הודעה אוטומטית עם פרטי ההרשמה; אפשר גם הודעת מדיה עם כתובת והגעה.
  5. "פולואפ" - 3 הודעות מעקב אוטומטיות ללידים שלא נרשמו או לא הגיבו (לא נשלח בלילות ובשבת, ומכבד מגבלת 24 שעות של מטא).

אוטומציות (טאב /templates):
...
- חיבור לאוטומציה (Zapier / webhook) — כפתור "איך מחברים לאוטומציה?" בעמוד.
- טריגרים אוטומטיים (דורשים חיבור Arbox לטריגרים מסוימים):
  · ליד מאתר/קמפיין — שליחת טמפלייט פתיחה לליד חדש.
  · ליד חדש מארבוקס — טמפלייט פתיחה לליד שנוצר בארבוקס (דורש חיבור Arbox).
  · חזרה אחרי שתיקה — הודעה לליד שלא הגיב.
  · רכישה, סירוב אשראי, נוכחות בשיעור ניסיון, יום הולדת, פג תוקף מנוי, פג תוקף כרטיסיה — טריגרים מבוססי Arbox.

נקודות ניווט חשובות:
- עריכת השיחה של הבוט מול הליד (השאלות, התגובות וה-flow) — בשלב "שיחה" בתוך "מסלול מכירה".
- הפעלה/כיבוי של זואי ומספר הוואטסאפ המחובר — בשלב "על העסק" בתוך "מסלול מכירה".
- הגדרת שיחת מכירה (במקום הרשמה לניסיון) — בשלב "שיחה" → הנעה לפעולה → "סוג כפתור 1".
- הגדרת טמפלייט פתיחה ללידים וטריגרים - בטאב "אוטומציות".
- הגדרת התראות לבעלים — בתפריט חשבון → "התראות".
...
export async function POST(req: NextRequest) {
  ...
  const asksAboutTemplates =
    (normalizedMessage.includes("טמפלייט") ||
      normalizedMessage.includes("template") ||
      normalizedMessage.includes("טריגר") ||
      normalizedMessage.includes("trigger") ||
      normalizedMessage.includes("אוטומציה") ||
      normalizedMessage.includes("zapier")) &&
    (normalizedMessage.includes("איך") ||
      normalizedMessage.includes("איפה") ||
      normalizedMessage.includes("מה זה") ||
      normalizedMessage.includes("ליד מאתר") ||
      normalizedMessage.includes("קמפיין"));

  const asksAboutNotifications =
    (normalizedMessage.includes("התרא") || normalizedMessage.includes("notification")) &&
    (normalizedMessage.includes("איך") ||
      normalizedMessage.includes("איפה") ||
      normalizedMessage.includes("להגדיר") ||
      normalizedMessage.includes("לקבל") ||
      normalizedMessage.includes("סיכום יומי") ||
      normalizedMessage.includes("נרשם") ||
      normalizedMessage.includes("נציג"));

  const asksAboutSalesCall =
    (normalizedMessage.includes("שיחת מכירה") ||
      normalizedMessage.includes("קביעת מועד לשיחה") ||
      normalizedMessage.includes("שיחה עם נציג")) &&
    (normalizedMessage.includes("איך") ||
      normalizedMessage.includes("איפה") ||
      normalizedMessage.includes("להגדיר") ||
      normalizedMessage.includes("להפעיל") ||
      normalizedMessage.includes("לשנות"));

  type DeterministicReply = { clean: string; model: string };
  let deterministic: DeterministicReply | null = null;

  if (asksWhereToEditBotConversation) {
    deterministic = {
      clean:
        'את השיחה של הבוט מול הליד - הפתיחה, השאלות, התשובות וההודעות - עורכים בשלב "שיחה" בתוך "מסלול מכירה". שם מנוסחות ההודעות שזואי שולחת ללידים.',
      model: "owner_help_deterministic_navigation",
    };
  } else if (asksAboutTemplates) {
    deterministic = {
      clean:
        'טמפלייטים וטריגרים מוגדרים בטאב "אוטומציות" בדשבורד. שם יוצרים תבניות וואטסאפ, מגדירים "טמפלייט הפתיחה ללידים", ומוסיפים טריגרים (למשל ליד מאתר/קמפיין). לחיבור לאוטומציה (Zapier וכו\') יש כפתור "איך מחברים לאוטומציה?" בעמוד.',
      model: "owner_help_deterministic_templates",
    };
  } else if (asksAboutNotifications) {
    deterministic = {
      clean:
        'התראות לבעל העסק (ליד נרשם, ביקש נציג, סיכום יומי) מוגדרות בתפריט החשבון (אייקון הפרופיל) → "התראות". אפשר לבחור וואטסאפ ו/או מייל. כדי לקבל התראות בוואטסאפ צריך לחבר את המספר שלכם בעמוד ההתראות.',
      model: "owner_help_deterministic_notifications",
    };
  } else if (asksAboutSalesCall) {
    deterministic = {
      clean:
        'כדי להגדיר "שיחת מכירה" במקום הרשמה לניסיון: מסלול מכירה → שלב "שיחה" → הנעה לפעולה → "סוג כפתור 1" → בחרו "שיחת מכירה". שם גם מסמנים את ימי השבוע ובלוקי השעות הזמינים. כשליד בוחר מועד, תקבלו התראה.',
      model: "owner_help_deterministic_sales_call",
    };
  }
  ...
  if (deterministic) {
    await admin.from("support_request_messages").insert({
      request_id: requestId,
      role: "assistant",
      content: deterministic.clean,
      model_used: deterministic.model,
    });
    await admin
      .from("support_requests")
      .update({ last_message_at: new Date().toISOString(), updated_at: new Date().toISOString() } as any)
