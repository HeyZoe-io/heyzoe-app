...
import { isAdminAllowedEmail, resolveSupabaseStorageBucket } from "@/lib/server-env";
...
/** העלאת מדיה לפלואו שיווקי (אדמין) — אותו דלי ומגבלה כמו `/api/dashboard/upload-media-signed-url`. */
export async function POST(req: NextRequest) {
  ...
  const trySign = () => admin.storage.from(bucket).createSignedUploadUrl(path, { upsert: true });
