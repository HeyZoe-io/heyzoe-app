...
type Step4SalesFlowProps = {
  ...
  setSalesFlowCallSchedulingEnabled?: (v: boolean) => void;
  callScheduleSlots?: Array<{ day_of_week: number; time_block: string }>;
  setCallScheduleSlots?: Dispatch<SetStateAction<Array<{ day_of_week: number; time_block: string }>>>;
...
export default function Step4SalesFlow(props: Step4SalesFlowProps) {
  const {
    ...
    salesFlowCallSchedulingEnabled = false,
  ...
  const firstSvcForWarmup = firstNamedService;
  ...
  }, [warmupSessionEnabled, salesFlowConfig]);

  const warmupQ1Active = isWarmupExperienceQuestion1Configured({
    question: salesFlowConfig.experience_question,
    options: salesFlowConfig.experience_options,
  });
            ...
            {!warmupSessionEnabled ? (
              ...
              <>
                ...
                <WarmupSessionQuestion1Block
                  ...
                  onQuestionChange={(v) => {
                    const sn = firstSvcForWarmup?.name?.trim() ?? "";
                    setSalesFlowConfig((c) => ({
                      ...c,
                    ...
                    <div className="space-y-1.5">
                      ...
                      {locked === "trial" ? (
                        <select
                          ...
                          aria-label={t.salesFlow.trialCtaMode}
                          value={salesFlowCallSchedulingEnabled ? "sales_call" : "registration"}
                          onChange={(e) => {
                            const next = e.target.value === "sales_call";
                            setSalesFlowCallSchedulingEnabled?.(next);
                            if (next) {
                              setSalesFlowConfig((c) => ({
                                ...c,
                                cta_buttons: c.cta_buttons.map((x) =>
                                  x.kind === "trial" ? { ...x, label: CALL_SCHEDULE_CTA_LABEL } : x
                                ),
                              ...
                              setSalesFlowConfig((c) => ({
                                ...c,
                                cta_buttons: c.cta_buttons.map((x) =>
                                  x.kind === "trial" && isCallScheduleCtaLabel(x.label)
                                    ? { ...x, label: "הרשמה לשיעור ניסיון" }
                                    : x
                                ),
                                followup_after_next_class_options: [
                                  isCallScheduleCtaLabel(c.followup_after_next_class_options[0] ?? "")
                                    ? "הרשמה לשיעור ניסיון"
                        ...
                        >
                          <option value="registration">{t.salesFlow.trialCtaModeRegistration}</option>
                          <option value="sales_call">{t.salesFlow.trialCtaModeSalesCall}</option>
                        </select>
                      ) : (
                        <>
                          <select
