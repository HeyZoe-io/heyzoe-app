 ...
 *
 * טוקן: WHATSAPP_TOKEN או META_ACCESS_TOKEN
 */
...
const token =
  process.env.WHATSAPP_TOKEN?.trim() || process.env.META_ACCESS_TOKEN?.trim() || "";

if (!token) {
  console.error("חסר טוקן: הגדר WHATSAPP_TOKEN או META_ACCESS_TOKEN ב-.env.local");
  process.exit(1);
}
